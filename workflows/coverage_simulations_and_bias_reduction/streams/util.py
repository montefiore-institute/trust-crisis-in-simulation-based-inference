...
from ratio_estimation import DatasetJointTestSmall as DatasetSmall
...
@torch.no_grad()
def coverage_of_estimator(path_to_weights, cl_list=[0.95], reduce="ratio_mean", flow_sbi=False, test=False, max_samples=None):
    if flow_sbi:
    ...
    if max_samples is None or test:
        inputs = d[:]["inputs"]
        outputs = d[:]["outputs"]
    else:
...
def importance_posterior_sampling(prior, posterior, nb_samples, nb_gen_samples):
    ...
    init_samples = init_samples.view(-1, 1)
...
def compute_sbc(path_to_weights, nb_rank_samples, nb_posterior_samples, save_name, reduce="ratio_mean", flow_sbi=False):
    ...
    if flow_sbi:
        ...
        class RatioPosterior():
            ...
            def log_prob(self, theta):
                outputs = self.x.repeat(len(theta), 1).float()
    ...
    sbc_run(prior, Dataset(), sample_posterior, nb_rank_samples, nb_posterior_samples, save_name)
