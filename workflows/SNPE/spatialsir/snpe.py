...
def build_embedding():
    ...
    return torch.nn.Sequential(
        torch.nn.Linear(3 * 50 * 50, hidden),
        torch.nn.SELU(),
    ...
    while not executed_without_errors and current_try < 10:
        try:
            ...
            for _ in range(num_rounds):
                ...
                density_estimator = inference.append_simulations(theta, x, proposal=proposal).train(learning_rate=lr, max_num_epochs=epochs, training_batch_size=batch_size)
...
def load_estimators_parameters_observables(query):
    ...
    observables = []
    for p in paths:
        path = os.path.join(p, "posterior.pkl")
        if os.path.exists(path):  # Check if the job has been completed.
            posteriors.append(pickle.load(open(os.path.join(p, "posterior.pkl"), "rb")))
            parameters.append(pickle.load(open(os.path.join(p, "theta_0.pkl"), "rb")))
            observables.append(pickle.load(open(os.path.join(p, "x_0.pkl"), "rb")))
...
def compute_log_posterior(posterior, observable, resolution=100):
    ...
    log_posterior = posterior.log_prob(inputs, x=observable, norm_posterior=False)[0]
...
def coverage(posteriors, nominals, observables, alphas=[0.05]):
    ...
    for posterior, nominal, observable in tqdm(zip(posteriors, nominals, observables), "Coverages evaluated"):
        ...
        nominal_pdf = posterior.log_prob(nominal, x=observable, norm_posterior=False).exp()
