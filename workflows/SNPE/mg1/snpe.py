...
@torch.no_grad()
def compute_log_posterior(posterior, observable, resolution=100, batch_size=10000):
    # Prepare grid
    p1 = torch.linspace(extent[0], extent[1], resolution)
    p2 = torch.linspace(extent[2], extent[3], resolution)
    p3 = torch.linspace(extent[4], extent[5], resolution)
    ...
    for b in range(0, inputs.shape[0], batch_size):
        ...
        tmp = posterior.log_prob(cur_inputs, x=observable, norm_posterior=False)[0]
        log_posterior[b:b+batch_size] = tmp

    assert (log_posterior.shape == (resolution**3,))
...
def coverage(posteriors, nominals, observables, alphas=[0.05]):
    ...
    for posterior, nominal, observable in tqdm(zip(posteriors, nominals, observables), "Coverages evaluated"):
        pdf = compute_log_posterior(posterior, observable, resolution=50).exp()
        nominal_pdf = posterior.log_prob(nominal, x=observable, norm_posterior=False).exp()
        for i, alpha in enumerate(alphas):
            level = highest_density_level(pdf, alpha, min_epsilon=10e-10)
