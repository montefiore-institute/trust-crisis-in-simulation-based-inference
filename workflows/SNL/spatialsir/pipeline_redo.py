...
from snl import build_posterior
...
# Simulation grid
#simulations = [32768, 131072]
#simulations = [1024, 2048, 4096, 8192, 16384, 32768, 65536, 131072]
...
def evaluate_snpe(simulation_budget):
    ...
    @w.dependency(main)
    @w.postcondition(w.exists(storagedir + "/run-00119/posterior.pkl"))
    @w.slurm.cpu_and_memory(4, "4g")
    @w.slurm.timelimit("72:00:00")
    @w.tasks(num_ensembles)
    def train_snpe(task_index):
        resultdir = storagedir + "/run-" + str(100+task_index).zfill(5)
        os.makedirs(resultdir, exist_ok=True)
        if not os.path.exists(os.path.join(resultdir, "posterior.pkl")):
            logging.info("Training posterior estimator ({index} / {n}) for the small SLCP problem.".format(index=100+task_index + 1, n=100+num_ensembles))
            logging.info("Using the following hyper parameters:")
            logging.info(" - batch-size     : " + str(batch_size))
            logging.info(" - epochs         : " + str(epochs))
            logging.info(" - learning-rate  : " + str(learning_rate))
            logging.info(" - simulations    : " + str(simulation_budget))
            build_posterior(simulation_budget, resultdir, learning_rate, epochs, batch_size, 100+task_index)
    ...
    def coverage():
        if not os.path.exists(storagedir + "/coverage.npy"):
            ...
            coverage = coverage_of_estimator(query)
    ...
    def mutual_information():
        if not os.path.exists(storagedir + "/mutual_information.npy"):
            query = storagedir + "/run-*/"
            mutual_information = mutual_information_of_estimator(query)
