...
def build_posterior(simulation_budget, out, lr, epochs, batch_size, task_index):
    prior = Prior()
    simulator = Simulator()
    prior._validate_sample = lambda x: True
    num_rounds = 10
    #num_rounds = 2
    ...
    while not executed_without_errors and current_try < 10:
        try:
            ...
            classifier_build_fun = classifier_nn(model='mlp', hidden_features=64)
            inference = SNRE(prior=prior, classifier=classifier_build_fun, summary_writer=log_writer)
...
def compute_log_posterior(posterior, observable, resolution=100, batch_size=10000):
    ...
    for b in range(0, inputs.shape[0], batch_size):
        ...
        tmp = posterior.log_prob(cur_inputs, x=observable)[0]
