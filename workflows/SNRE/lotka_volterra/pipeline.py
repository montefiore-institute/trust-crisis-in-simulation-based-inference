...
from snre import build_posterior
...
def evaluate_snpe(simulation_budget):
    ...
    def train_snpe(task_index):
        ...
        if not os.path.exists(os.path.join(resultdir, "posterior.pkl")):
            logging.info("Training posterior estimator ({index} / {n}) for the Lotka Volterra problem.".format(index=task_index + 1, n=num_ensembles_gen))
