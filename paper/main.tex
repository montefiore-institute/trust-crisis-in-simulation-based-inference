...
\newcommand{\old}[1]{\textcolor{magenta}}
...
%   In recent years a myriad of simulation-based inference algorithms have been developed
%   with the objective to produce approximations as close as possible to the ground truth.
%   In this work we take the position that minimizing a divergence between the approximation and an unknown ground truth can incorrectly drive scientific inquiry. Instead, we argue for conservativeness. Approximations should therefore always convey at least as much uncertainty as the ground truth, and never be overconfident. We assess conservativeness by measuring coverage and showcase through extensive experiments that various approaches in simulation-based inference can produce non-conservative posterior estimators. We show empirically that ensembles consistently help in this regard but can still lead to non-conservative posterior estimators. Finally, we discuss several perspectives and open avenues of research towards conservative estimators.

% My pass:
%
%
%Computer simulators are at the center of many scientific disciplines.
%Although modern simulators are able to describe complex processes, they are less suited
%for statistical inference. 
%At the core of this problem lies the intractable likelihood model implicitly defined by the simulator's computer code. 
%To deal with this intractability,
%a myriad of Bayesian inference algorithms have been developed to approximate
%posteriors as possible to their unknown ground truths. The only requirement of these algorithms being that generating simulations is possible.
%In this work, we take the position that minimizing a divergence between the approximation and an unknown ground truth can incorrectly drive scientific inquiry. Instead, we
%argue for conservative approximations, meaning that they should always convey at least as much uncertainty as the ground truth. We collect extensive
%empirical evidence spanning months of computation on several computer clusters. Our results indicate that all benchmarked techniques produce non-conservative posterior
%approximations. Complicating their applicability.
%We show that ensembles are consistently more reliable in this regard, but can still lead to non-conservative
%estimators. Our study is complemented with new research proposals to strengthen the
%application of simulation-based inference in the sciences.

%\joeri{Reworked Gilles' proposal a bit. See below.}

% 
% Joeri's proposal after Gilles' proposal
%
%

%Recent years have seen tremendous innovations in the field of Bayesian simulation-based inference.
%{\color{blue} This work takes a step back by assessing the reliability of these algorithms to scientific use-cases concerned with constraining parameters, as is often the case in the physical sciences.??? We present...}
%This work presents extensive empirical evidence that all benchmarked algorithms -- including \textsc{(s)nre}, \textsc{(s)npe}, \textsc{snl} and various forms of \textsc{abc} -- produce non-conservative posterior approximations. Making these algorithms demonstrably unreliable if one's scientific goal is to constrain parameters of interest, as is often the case in the physical sciences. Our results further demonstrate that ensembles are consistently more reliable in this regard, although they may still lead to non-conservative posterior approximations.
%We argue that failing to address this issue will lead to a well-founded distrust in simulation-based inference. For this reason we argue that research efforts should now focus on theoretical and methodological advances to guarantee conservative approximate inference.
...
% \\\joeri{Which version? Merge?}

% In recent years a myriad of simulation-based inference algorithms have been developed to perform Bayesian inference with an intractable likelihood implicitly defined by a simulator. Those algorithms traditionally have as objective to produce approximate posterior over the quantity to be inferred as close as possible to the ground truth posterior. In this work, we take the position that minimizing a divergence between the approximation and an unknown ground truth can lead to overconfident approximations and hence incorrectly drive scientific inquiry. Instead, we argue for conservativeness meaning that approximations should therefore always convey at least as much uncertainty as the ground truth, and never be overconfident. Our main contribution lies in the generation through month of computation of extensive empirical evidence that all currently existing approaches in simulation-based inference can produce non-conservative posterior estimators and are hence not suited for scientific purpose as they are. We show empirically that ensembles consistently help in this regard but can still lead to non-conservative posterior estimators and discuss several perspectives and open avenues of research towards conservative estimators.
...
% Although modern simulators can generate realistic synthetic observables through detailed descriptions of their data generating processes, 
% they are unfortunately not suitable for statistical inference.
% At the core of this issue lies the likelihood model $p(\vx\vert\vtheta)$ implicitly defined by 
% computer code describing the data generating processes.
% The direct evaluation of the likelihood rests on the \emph{intractable} integration of all stochastic execution paths, rendering statistical inference based on the likelihood impractical.
% Fortunately, inference remains possible by relying on likelihood-free \emph{approximations} \citep{cranmer2020frontier}
% within a field referred to as simulation-based or likelihood-free inference. 
...
%\href{https://github.com/JoeriHermans/troubles-in-paradise}{\texttt{github.com/montefiore-ai/troubles-in-paradise}}


%While the majority of machine learning literature targets to \emph{maximize} a criterion such as accuracy or likelihood with the objective of obtaining the best fit, the natural sciences make decisions based on frequentist confidence intervals or Bayesian credible regions. 

%Although simulation-based inference targets domain sciences, advances in the field are mainly driven from a machine learning perspective.
%The field therefore inherits the quality assessments \citep{lueckmann2021benchmarking} customary to the machine learning literature which usually involves maximizing some criterion. Despite recent developments of post hoc diagnostics to inspect the quality of likelihood-free approximations \citep{cranmer2015approximating, Brehmer:2018eca, brehmer2019mining, Hermans:2020skz, lueckmann2021benchmarking, sbc, dalmasso2020confidence}, the problem concerning \emph{scientific conclusions} based on simulation-based inference remains: \emph{can these techniques be used to drive scientific research whenever their fitting criteria are not globally optimized or the availability of data is limited?}

%{\bfseries sciences are rather concerned with
%\emph{excluding} hypotheses at some confidence level}

%As an illustration of this, a recent application consist in inferring the nature (cold or warm) and mass of dark matter through the observation of the evolution of stellar streams' density. The simulator evolves a stream and its stellar density through a stochastic iterative process though interactions with a family of dark matter subhaloes parametrized though the mass of the dark matter \arnaud{where does the nature appear?}. The iterative nature of the simulator leads to an intractable likelihood since evaluating it would require to consider all execution paths. \joeri{Rewrite}

%While the majority of machine learning literature targets to \emph{maximize} a criterion such as accuracy or likelihood with the objective of obtaining the best fit, the natural {\bfseries sciences are rather concerned with
%\emph{excluding} hypotheses at some confidence level}
%While the majority of machine learning literature targets to \emph{maximize} a criterion such as accuracy or likelihood with the objective of obtaining the best fit, a common concern in natural {\bfseries sciences is rather to \emph{exclude} hypotheses at some confidence level}.

%For this reason, domain scientists want guarantees to whether statistical \emph{approximations} have confidence intervals (frequentist) or credible regions (Bayesian) not smaller than they should be. In this work we adopt the Bayesian point of view, and are then concerned with not rejecting viable parameters from the credible region and incorrectly drive scientific inquiry.
%If that would \emph{not} be the case, scientists could reject viable hypotheses and incorrectly drive scientific inquiry.

%Considering the example of the inference of the nature and mass of dark matter based on the observation of a stellar streams, incorrectly inferring the nature can have a disastrous cascading effects. Those can be found in the field of particle physics where excluding warm dark matter would exclude the hypothesized sterile neutrino. \arnaud{refs} On the contrary, if cold dark matter were to be excluded, the theory for the disruptions of stellar streams would not hold.


%{\bfseries what guarantees or properties do these techniques have whenever their fitting criteria are not globally optimized or the availability of data is limited?}


% Every result and figure is annotated with \code{https://github.com/montefiore-ai/troubles-in-paradise}, linking to the code used to generate it.

%Despite recent methodological developments~\gilles{refs} and successes across various scientific use cases~\gilles{refs}, a major criticism of simulation-based inference is that the involved statistical computations are essentially not verifiable and are therefore not suitable for scientific disciplines whose primary objective is to \emph{constrain} parameters of interest, or \emph{exclude} models at a certain confidence level.

%We emphasize that algorithms should not just show high performances \arnaud{rephrase}. In the case of Bayesian posterior inference, domain scientists should also have guarantees that credible regions identified with approximate posteriors are not smaller than they should be, since rejecting parameter values compatible with the data will drive the scientific inquiry to wrong conclusions. \arnaud{this is the case with ... concrete example, gw, streams and refs}

%These concerns have led to further work focusing on improving the accuracy of inference procedures~\gilles{refs} and to the development of post hoc diagnostics to inspect the quality of their approximations~\citep{cranmer2015approximating, Brehmer:2018eca, brehmer2019mining, Hermans:2020skz, lueckmann2021benchmarking}.
%Despite these innovations, it remains unanswered what formal guarantees these techniques have whenever their approximations are not optimal, or equivalently, whenever their fitting criteria are not globally optimized.

%Theoretical characterizations of simulation-based techniques are critically needed to draw scientific conclusions reliably. 
%In this work, we study the empirical coverage of various simulation-based inference techniques over multiple benchmarks and first demonstrate that all those techniques may lead to credible regions that are too narrow with respect to the ground truth, hence leading to Type-I errors and possibly to wrong scientific conclusions. 
%We argue that model regularization constitutes a potential solution and provide a critical discussion about the ability to diagnose such pathologies. The paper is outlined as follows. \arnaud{Update paper structure} Section~\ref{sec:background} outlines the statistical formalism and the necessary background of simulation-based inference. We briefly highlight the importance of a proper diagnostic to inspect the statistical quality of the approximations. Section~\ref{sec:method} outlines our theoretical results and proposed methodology, supported by experimental validation in Section~\ref{sec:experiments}.
\section{Background}
...
%which can either be the posterior itself. Or the likelihood-to-evidence ratio 
%$$r(x\vert\vartheta) = \frac{p(x \vert \vartheta)}{p(x)}.$$ 
%Through approximations $\hat{r}(x\vert\vartheta)$, evaluating estimates of the posterior density function amount to computing the product of the \emph{tractable} prior probability $p(\vartheta)$ and the estimated likelihood-to-evidence ratio $\hat{r}(x\vert\vartheta)$. An unormalized approximate of the posterior can also be recovered from a surrogate for the likelihood by multiplying it to the prior probability.

%\subsection{Likelihood-free inference methods}
\subsection{Statistical quality assessment}
%The work of \citet{lueckmann2021benchmarking} benchmarks common simulation-based
%techniques on various metrics. Although the benchmarks are done very thoroughly with respect to the \emph{selected} metrics, 
%there are issues regarding their \emph{applicability}.
...
%\begin{equation}
%\mathbb{E}_{(\vtheta^*, \vx_o) \sim p(\vtheta, \vx)} \left\{ \begin{array}{ll}
%    1 & \text{if } \vtheta^* \in \Theta_{\hat{p}(\vtheta|\vx = \vx_o)}(1 - \alpha) \\
%    0 & \text{otherwise}
%\end{array} \right..
%\end{equation}
\end{definition}
...
\end{definition}
% \begin{definition}
% The {empirical \bfseries coverage probability} of a posterior estimator at a confidence level $1 - \alpha$ given a set of $n$ i.i.d. samples $(\vtheta^*,\vx)\sim p(\vtheta,\vx)$
% is
% \begin{equation}
% \frac{1}{n} \sum_{i=1}^n \left\{ \begin{array}{ll}
%     1 & \text{if } \vtheta^*_i \in \Theta_i \\
%     0 & \text{otherwise}
% \end{array} \right.,
% \end{equation}
% where $\Theta_i$ is the credible region computed through the posterior approximation $\hat{p}(\vtheta\vert\vx=\vx_i)$ considering that $\vx_i \sim p(\vx\vert\vtheta = \vtheta^*_i)$.
% \end{definition}
% \begin{definition}
% The {\bfseries nominal coverage probability} is effectively equal to the confidence level.
% \end{definition}
% \begin{definition}
% An estimator is deemed acceptable if it {\bfseries has coverage} at the confidence level of interest. That is, whenever the empirical coverage probability is {\bfseries larger or equal} to the nominal coverage probability.
% \end{definition}
% \begin{definition}
% A {\bfseries conservative estimator} is an estimator that has coverage for {\bfseries all} confidence levels.
% \end{definition}
...
%from  posterior $p(\vartheta|x_0)$ for an observable $x_0$. Candidates parameters $\vartheta_i$ are first sampled from the prior, $p(\vartheta)$. Those parameters are then used along with the simulator to produce observations $x_i$. The candidate parameters that have produced the closest observations to the target observation $x_0$ according to some distance metric are kept and considered samples from the posterior, $p(\vartheta|x_0)$. \arnaud{add details?: norm 2 distance + quantile 0.01 with at least 100 samples?}  
%Sequential Monte-Carlo ABC (SMC-ABC) \citep{10.1093/bioinformatics/btp619, sisson2007sequential, beaumont2009adaptive} iteratively update a set of particules to match the posterior distribution. At each iteration, candidate parameters that produced the highest distance are discarded and new candidates are generated by adding a perturbation to the remaining candidates. \arnaud{double check that this is correct}
...
% Note that
% amortized estimates of the likelihood-to-evidence ratio $r(x\vert\vartheta)$ can be computed through $\hat{p}_\psi(\vartheta\vert x)$ because
% $\log r(x\vert\vartheta) \approx \log \hat{p}_\psi(\vartheta\vert x) - \log p(\vartheta)$. Therefore, the binary discriminator $\hat{d}(\vartheta,x)\triangleq\sigmoid(\log\hat{p}_\psi(\vartheta\vert x) - \log p(\vartheta))$ enables $\hat{p}_\psi(\vartheta,x)$ to be {\bf learned by minimizing
% the binary cross entropy criterion} presented above. All flow-based estimators in this work are optimized using the aforementioned criterion unless stated otherwise.
% \joeri{We can remove this right? Yes my bad}
% Once trained, we can sample new data points from the modeled distribution as well as evaluate the density of new data points. We highlight two possibilities for building an approximate amortized posterior $p_\psi(\vartheta|x)$ with NFs. First, by direct minimization of the \textsc{kl} divergence between the approximate posterior and the actual posterior,
% \begin{align}
%     \psi^* &= \argmin_\psi \mathbb{E}_{p(x)}\left[\textsc{kl}(p(\vartheta|x)~\vert\vert ~p_\psi(\vartheta|x)\right] = \argmin_\psi \mathbb{E}_{p(x)}\mathbb{E}_{p(\vartheta|x)}\left[\log\frac{ p(\vartheta|x)}{p_\psi(\vartheta|x)}\right]\\
%     &= \argmax_\psi \mathbb{E}_{p(\vartheta,x)}\left[\log p_\psi(\vartheta|x)\right].
% \end{align}
% \antoine{Say it is equivalent to MLE. (amortized over posteriors)}
% The second option is to define the NF likelihood-to-evidence ratio as 
% \begin{align}
%     r_\psi(x|\vartheta) := \frac{p_\psi(\vartheta|x)}{p(\vartheta)},
% \end{align}
% and to train it as a classifier-based amortized ratio estimator.
% \antoine{Add the two training methods have the same optimum but the second may be tuned to be blablabla as shown later}
% \joeri{My take below?}

%{\bfseries Sequential methods} aim at estimating the posterior for a specific observable $x_o$ by dedicating the simulation budget to iteratively improve an approximation.
%These methods alternate between a training and a simulation phase, the latter being designed to take \emph{current} knowledge into account such that subsequent simulation parameters $\vartheta$ are expected to produce observables $x$ closer to $x_o$ compared to previous iterations. 
...
\end{figure*}
\begin{figure*}[h!]
    %\includegraphics[width=\textwidth]{figures/coverage_multi_ensemble.pdf}
...
% In Figure \ref{fig:coverage_ensemble} we observe that the empirical coverage probability of the ensemble models is consistently larger than the empirical coverage probability of the expected individual estimator. Individual estimators only captures aleatoric uncertainty, i.e., uncertainty that is linked to stochasticity of the data generation procedure. Ensembles are expected to also capture part of the epistemic uncertainty, i.e., uncertainty issued by the lack of knowledge. In particular, ensembles capture the uncertainty related to the training procedure. This added uncertainty results in a larger credible region increasing the empirical coverage probability. Although ensembles take part of the epistemic uncertainty into account, they can still produce non-conservative posteriors. 
% Current applications of simulation-based inference can therefore rely on ensembling to build increasingly conservative posterior estimators.
% It should be noted however that bagging does not yield a similar effect.
% As shown in Appendix \ref{sec:additional_results}, a positive effect with respect to ensemble size is observed.
...
% Our results show that ensembling constitutes a promising research direction. A theoretical understanding for the observation that ensembles produce increasingly conservative posteriors would further strengthen their applicability. In addition, the idea of capturing epistemic uncertainty with ensembles could further be extended using the \emph{Bayesian Deep Learning} framework, which attempts to model an approximate posterior over the weights of a neural network given the available data. The Bayesian framework assigns a clear interpretation to those uncertainties because they arise from the prior and likelihood. On the other hand, the uncertainty in ensembles arise from the stochasticity during training.
% In addition, the idea of capturing epistemic uncertainty with ensembles could further be extended using the \emph{Bayesian Deep Learning} framework, which attempts to model an approximate posterior over the weights of a neural network given the available data. In the likelihood-free setting, this data would be issued from the simulator. The posterior for a given observable would subsequently be obtained by marginalizing the posterior over the available weights. As with ensembles, the posterior over the weights takes epistemic uncertainty into account, which is the uncertainty issued by the lack of knowledge. The Bayesian framework assigns a clear interpretation to those uncertainties because they arise from the prior and likelihood. The uncertainty in ensembles on the other hand arise from the stochasticity in the training procedure.
